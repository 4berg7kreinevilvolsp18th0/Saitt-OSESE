"""
Полнотекстовый поиск для обращений и контента
"""
...
def search_appeals(
    ...
    # PostgreSQL array search (если теги содержат запрос)
    # Note: This is a simple implementation. For better performance, use PostgreSQL full-text search (например, для лучшей производительности используйте PostgreSQL полнотекстовый поиск)
...
def search_content(
    ...
    """
    Полнотекстовый поиск в контенте по заголовку и теле
    """
    search_filter = or_(
...
def search_appeals_by_tags(
    ...
    """
    Поиск обращений по тегам (PostgreSQL GIN array overlap contains operator) (пересечение массивов) (например, для поиска обращений по тегам)
    """
    # PostgreSQL GIN array overlap contains operator (пересечение массивов) (например, для поиска обращений по тегам)
    from sqlalchemy.dialects.postgresql import array
    db_query = db.query(Appeal).filter(Appeal.tags.overlap(tags))
    return db_query.order_by(Appeal.created_at.desc()).offset(skip).limit(limit).all()
