...
export default function DirectionCard({ d }: { d: Direction }) {
  ...
  return (
    ...
    >
      <div className={`text-xs sm:text-sm uppercase tracking-wide ${accent.split(' ')[0]}`}>
        {d.title}
      </div>
      <div className="mt-2 text-white/80 text-xs sm:text-sm leading-relaxed">{d.description}</div>
      <div className="mt-4 sm:mt-5 inline-flex items-center gap-2 text-xs sm:text-sm text-white/70">
        Перейти
