...
export default function DirectionCard({ d }: { d: Direction }) {
  ...
  const titleColorClass =
    d.colorKey === 'legal'
      ? 'text-blue-50 light:text-legal-dark-blue'
      : d.colorKey === 'infrastructure'
        ? 'text-cyan-50 light:text-blue-800'
        : d.colorKey === 'scholarship'
          ? 'text-emerald-50 light:text-emerald-800'
          : d.colorKey === 'international'
            ? 'text-yellow-50 light:text-amber-800'
            : 'text-slate-50 light:text-slate-800';
  ...
  return (
    ...
    >
      <div className="relative z-10 flex-1 flex flex-col">
        ...
        <div
          className={`inline-flex items-center rounded-md px-2.5 py-1 relative text-sm sm:text-base uppercase tracking-wider font-bold mb-3 ${titleColorClass}
            bg-black/35 light:bg-white/85
            shadow-[0_1px_10px_rgba(0,0,0,0.35)] light:shadow-sm`}
        >
          {d.title}
        </div>
        <div
          className={`text-sm sm:text-base leading-relaxed font-sf-text mb-4 flex-1 text-white light:text-gray-700 light:font-medium
            drop-shadow-[0_1px_8px_rgba(0,0,0,0.35)] light:drop-shadow-none`}
        >
          {d.description}
        </div>
        <div className="inline-flex items-center gap-2 text-sm sm:text-base font-semibold text-white/95 light:text-gray-800 group-hover:gap-3 transition-all mt-auto">
          Перейти
          <span
